...
async function doRestoreSession(set) {
...
async function doSignIn(set, email, password) {
  const session = await cognitoSignIn(email, password);
  const parsed = parseIdToken(session);
  set({ user: parsed, token: session.getIdToken().getJwtToken() });
}

function doSignOut(set, get) {
  cognitoSignOut();
  set({
    user: null,
    token: null,
...
export const createAuthSlice = (set, get) => ({
  ...
  signIn: (email, password) => doSignIn(set, email, password),
