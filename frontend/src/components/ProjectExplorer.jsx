    ...
    <div className="explorer">
      ...
      <div className="explorer-tree">
        {projects.map((project) => {
          ...
          const subs = subsystems[project.id];
          const isLoading = loadingSubs.has(project.id);
          const viewPath = `/view/${project.id}`;

          return (
            <div key={project.id} className="explorer-node">
              <div className={`explorer-row${isActive(viewPath) ? " active" : ""}`}>
                ...
                <button type="button" className="explorer-name" onClick={() => navigate(viewPath)}>
              ...
              {isExpanded && (
                <div className="explorer-children">
                  ...
                  {subs &&
                    subs.map((sub) => {
                      const subPath = `/view/${project.id}/${sub.id}`;
                      return (
                        <div
                          key={sub.id}
                          className={`explorer-row explorer-sub${isActive(subPath) ? " active" : ""}`}
                        >
                          <button
                            type="button"
