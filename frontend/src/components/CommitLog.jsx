    ...
    <div className="commit-log">
      ...
      {isOpen && (
        <div className="commit-log-list">
          ...
          {relevantCommits
            .slice(0, 5)
            .map((commit, index) => renderCommitEntry(commit, index, relevantCommits))}
          {relevantCommits.length > 0 && (
            <button
              type="button"
              className="ghost commit-view-all"
        ...
        <div className="commit-full-overlay" onClick={() => setShowFullHistory(false)}>
          <div className="commit-full-modal" onClick={(e) => e.stopPropagation()}>
            <div className="commit-full-header">
              ...
              <button type="button" className="ghost" onClick={() => setShowFullHistory(false)}>
                Close
              </button>
            </div>
            <div className="commit-full-list">
              {relevantCommits.map((commit, index) =>
                renderCommitEntry(commit, index, relevantCommits)
              )}
            </div>
          </div>
        </div>
