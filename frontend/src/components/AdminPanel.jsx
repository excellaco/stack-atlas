      ...
      <div className="admin-panel" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="panel-header">
          ...
          <div className="admin-header-actions">
            <button type="button" className="ghost" onClick={handleDownloadCatalog}>
              Download Catalog
            </button>
            ...
            <section className="admin-section">
                      ...
                      return (
                        <div key={sub} className="admin-list-item">
                          ...
                          <button
                            type="button"
                            className="ghost danger"
                            onClick={() => removeEditor(projectId, sub)}
                          >
                            Remove
                          </button>
                        </div>
              ...
              <div className="admin-add-row">
                <select
                  ...
                  onChange={(e) => setNewEditorProject(e.target.value)}
                >
          ...
          <>
            <section className="admin-section">
              <div className="catalog-actions">
                ...
                <button type="button" className="ghost" onClick={handleSeedFromStatic}>
                  Seed from Static
                </button>
                <input
                  ref={fileInputRef}
            ...
            <section className="admin-section">
                ...
                <div className="catalog-form">
                  ...
                  <div className="catalog-form-actions">
                    <button
                      type="button"
                      className="primary"
                      onClick={() => handleSaveCatalogCategory(editingCategory)}
                      disabled={!editingCategory.id?.trim() || !editingCategory.name?.trim()}
                    >
                      Save
                    </button>
                    <button
                ...
                {editCategories.map((cat) => (
                  <div key={cat.id} className="admin-table-row">
                    <div className="admin-table-main">
                      ...
                      {cat.description && (
                        <span className="admin-table-desc">{cat.description}</span>
                      )}
                    </div>
                    <span className="admin-table-meta">
                      {editItems.filter((i) => i.category === cat.id).length} items
                    </span>
                    <button
                      type="button"
                      className="ghost"
                    ...
                    <button
                      type="button"
                      className="ghost danger"
                      onClick={() => handleDeleteCatalogCategory(cat.id)}
                    >
                      Delete
                    </button>
                  </div>
              ...
              <button
                ...
                className="ghost"
                onClick={() =>
                  setEditingCategory({ id: "", name: "", description: "", color: "#6b7280" })
                }
              >
                + Add Category
              </button>
                  ...
                  <div key={item.id} className="admin-table-row">
                    <div className="admin-table-main">
                      ...
                      <span className="admin-table-desc">
                        {item.type} ·{" "}
                        {editCategories.find((c) => c.id === item.category)?.name || item.category}
                      </span>
                    </div>
              ...
              <div className="admin-section">
                    ...
                    {subsystems.map((s) => (
                      <div key={s.id} className="admin-table-row">
                        <div className="admin-table-main">
                          ...
                          {s.description && (
                            <span className="admin-table-desc">{s.description}</span>
                          )}
                        </div>
                        <button
                          type="button"
                ...
                {showCreateSub ? (
                  <form className="project-create-form" onSubmit={handleCreateSub}>
                    ...
                    <button type="button" className="ghost" onClick={() => setShowCreateSub(false)}>
                      Cancel
                    </button>
                  </form>
                ) : (
                ...
                {locks.map((lock) => (
                  <div key={`${lock.projectId}-${lock.userSub}`} className="admin-table-row">
                    ...
                    <button
                      type="button"
                      className="ghost danger"
                      onClick={() => handleBreakLock(lock.projectId, lock.userSub)}
                    >
                      Break
                    </button>
                  </div>
            ...
            {activity.length === 0 ? (
              ...
              <div className="commit-log-list">
                {activity.map((commit, index) => {
                  ...
                  const prevSnapshot =
                    index < activity.length - 1 ? activity[index + 1]?.snapshot : null;
                      ...
                      >
                        ...
                        <div className="commit-entry-body">
                          <div className="commit-entry-message">
                            <span className="commit-project-tag">{commit.projectName}</span>{" "}
                            {commit.message}
                          </div>
