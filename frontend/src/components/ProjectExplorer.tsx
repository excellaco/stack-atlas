...
function StackIcon({ className }: Readonly<IconProps>): React.JSX.Element {
  return (
    <svg
      className={className}
      width="14"
...
function SubIcon({ className }: Readonly<IconProps>): React.JSX.Element {
  return (
    <svg
      className={className}
      width="12"
...
function EditBadge(): React.JSX.Element {
  return (
...
function SubsystemRow({
  ...
  isActive,
  onNavigate,
}: Readonly<SubsystemRowProps>): React.JSX.Element {
...
function ProjectNode({
  ...
  onToggle,
  onNavigate,
}: Readonly<ProjectNodeProps>): React.JSX.Element {
...
function useExpandableProjects(token: string | null) {
  const [expanded, setExpanded] = useState<Set<string>>(new Set());
  const [subsystems, setSubsystems] = useState<Record<string, Subsystem[]>>({});
  const [loadingSubs, setLoadingSubs] = useState<Set<string>>(new Set());

  const toggleExpand = async (projectId: string): Promise<void> => {
...
export default function ProjectExplorer(): React.JSX.Element {
